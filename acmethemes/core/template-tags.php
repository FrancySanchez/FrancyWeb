...
function read_more_posted_on( $avatar_size = 60 ) {
	...
	$posted_on = sprintf(
		esc_html_x( '%s', 'post date', 'read-more' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><i class="fa fa-clock-o"></i>' . $time_string . '</a>'
	);
	$byline = sprintf(
		esc_html_x( '%s', 'post author', 'read-more' ),
		'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">'. get_avatar( get_the_author_meta( 'user_email' ), $avatar_size, '' ) . "<span class='author-name'>".esc_html( get_the_author() ) . '</span></a></span>'
	);
...
function read_more_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( '%1$s', 'read-more' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
}
endif;
	...
	function read_more_entry_cats() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && read_more_categorized_blog() ) {
				printf( '<span class="cat-links">' . esc_html__( '%1$s', 'read-more' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
		}
	}
endif;
